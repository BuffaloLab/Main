...
#2.) For the training steps, use only one banana rather than alternate between the three banana models on
#    left, right, and center, and adjust the corresponding functions appropriately.  Just change the positions
#    of the single banana.
#3.) Use assignment operators where possible
#4.) Use modulus if it would improve anything.
#5.) Use words for the logical operators for the sake of readability.
#6.) Use membership operators where possible, if they would improve anything.
...
if (training > 2) & (training < 2.5):
        ...
        targetHwinL = 4 #1.2 is sort of the boundary.
        targetHwinR = 4 #1.2 is sort of the boundary.
