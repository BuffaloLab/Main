...
class bananarchy:
	...
	if self.bananaModels[0].isStashed() == 0:
		...
		self.targetHeader = float('nan')
                ...
                self.bananaModels[0].setPos(Point3(random.uniform(-config['minDistance'], -config['maxDistance']), 2, 1))
	...
	if self.bananaModels[1].isStashed() == 0:
		...
		self.bananaModels[1].setStashed(0)
                a = self.bananaModels[1].getPos()[0]
                b = self.bananaModels[1].getPos()[1]
                self.targetHeader = rad2deg(arccos(b/sqrt(square(a) + square(b))))
	...
	if self.bananaModels[2].isStashed() == 0:
		...
		self.targetHeader = float('nan')
                ...
                self.bananaModels[2].setPos(Point3(random.uniform(config['minDistance'], config['maxDistance']), 2, 1))
    ...
    def trackHeader(self, config, avatar):
        ...
        if (hedder < self.targetHeader + config['targetHwinL']) & (hedder > self.targetHeader - config['targetHwinR']):
            ...
            if (int(config['training']) == 1) or (config['training'] == 3.2) or ((config['training'] >= 3.3) and (avatar.getPos()[1] > 0)):
