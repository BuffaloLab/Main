...
class Model(ModelBase, UniquelyIdentifiable):
    """
    An object converted and loaded from a 
    3D modeling package.
    """

    Log.getInstance().addType("MODEL_INIT", [('identifier',basestring), ('filename',basestring)])
    Log.getInstance().addType("MODEL_CREATED", [('identifier',basestring)])

    def __init__(self, identifier, model, location=None, callback=None):
        """
        ...
        """
        Log.getInstance().writeLine((mstime(), 0), "MODEL_INIT", [identifier, model])
        UniquelyIdentifiable.__init__(self, identifier)

        nodePath = base.loader.loadModel(model)
        ModelBase.__init__(self, identifier, nodePath, location, callback)
        VLQ.getInstance().writeLine("MODEL_CREATED", [identifier])
