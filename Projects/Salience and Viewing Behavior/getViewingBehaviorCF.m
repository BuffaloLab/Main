...
for cndlop = 1:2:length(fixationstats); %only uses novel viewing since images changes could alter natural behavior
    ...
    xy =fixationstats{cndlop}.XY;
    sacduration(reindexed,1:length(saccadetimes)) = diff(saccadetimes,1)';
    fixduration(reindexed,1:length(fixationtimes)) = diff(fixationtimes,1)';
    for i = 1:size(fixations,2)-1
        ...
        if i == 1;
            ...
            timebtwfix(reindexed,i) = (fixationtimes(2,i+1)-(fixationtimes(2,i+1)-fixationtimes(1,i+1))/2) ...
                -(fixationtimes(2,i)-(fixationtimes(2,i)-fixationtimes(1,i))/2);
        else
            densitymap(fixy,fixx) = densitymap(fixy,fixx)+1;
            timebtwfix(reindexed,i) = (fixationtimes(2,i+1)-(fixationtimes(2,i+1)-fixationtimes(1,i+1))/2) ...
                -(fixationtimes(2,i)-(fixationtimes(2,i)-fixationtimes(1,i))/2);
    ...
    for i = 1:size(saccadetimes,2)
        sacx = xy(1,saccadetimes(1,i)-1:saccadetimes(2,i)+1);
        sacy = xy(2,saccadetimes(1,i)-1:saccadetimes(2,i)+1);
        sacdist(reindexed,i) = sum(sqrt(diff(sacx).^2+diff(sacy).^2));
        sacangle(reindexed,i) = atan2(diff(sacy(1:2)),diff(sacx(1:2)));
        sacangle_2fix(reindexed,i) =  atan2(diff(sacy(end-1:end)),diff(sacx(end-1:end)));
    end
end
...
save([FIXATIONFILE(1:end-13) '-ViewingBehavior'],'fixduration',...
    ...
    'Statsbyfixation','distanceprofile','sacangle_2fix')