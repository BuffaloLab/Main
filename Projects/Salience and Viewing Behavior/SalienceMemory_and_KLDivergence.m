                ...
                if length(fixationstats) >= 2*i
                    ...
                    if length(fixations) >= 5;
                        ...
                        KLshuff(index,1,t) = sum(sum(log2(prefixshuffPDF15./postfixshuffPDF15).*prefixshuffPDF15))...
                            +sum(sum(log2(postfixshuffPDF15./prefixshuffPDF15).*postfixPDF15));
                        if ii >= 10;
...
%%
load(['C:\Users\GOD-OF-ChAOS\Documents\MATLAB\Buffalo Lab-Salience Model\SCM Image Sets\'...
...
hold on
errorbar(nanmean(nanmean(novsalfix(:,1:50,:),1),3),novstd);
errorbar(nanmean(nanmean(repsalfix(:,1:50,:),1),3),repstd,'r');
plot(1:50,allstatistics.confidenceintervals(1,1,1)*ones(1,50),'k--');
for i = 1:50;
    if pvalues(i) < 0.05
...
%%
figure
suptitle('Novel viewiing Fixation PDFs')
subplot(2,3,1)
