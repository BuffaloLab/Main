...
for imset = 1:length(image_sets);
    ...
    for i = 1:length(eyeindexes)
        ...
        for cndlop = 1:length(eyedat)
            ...
            for ii = 1:size(saccadetimes,2);
                ...
                elseif ii == size(saccadetimes,2);
                    if saccadetimes(2,ii) < length(accel);
                        fixationtimes(:,ii) = [saccadetimes(2,ii)+1,length(accel)];
                    else
            ...
            if max(max(saccadetimes)) > length(x) || max(max(fixationtimes)) > length(x)
                disp('overflow')
            end
        ...
        save([cortexfile(1:end-2) '_' cortexfile(end) '-thresh-fixation.mat'],'fixationstats')
    end
end