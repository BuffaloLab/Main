%% MAKES SETS # 300-399,  offset .4 and .2,  60 total blocks
...
% start posnum for these stimuli 300-399
numlist=(numlist(numlist<400 & numlist>299));

if ~isempty(numlist)
    posnumdum=num2str(max(numlist)+1);
    if (max(numlist)+1)==400
...
for l = 1:732 %change last number to change # of screens produced
    ...
    for poslop=1:size(pos,1)-1
        ...
        Larrx=[r (r+.6) (r+.4) (r+.4) ; (r+.6) (r+.6) r (r+.6); r (r+.6) r r; r (r+.6) r r];
        Larry=[p p p (p-.6); (p-.6) p (p-.4) (p-.4); (p-.4) (p-.4) p (p-.6); (p-.2) (p-.2) p (p-.6)];
