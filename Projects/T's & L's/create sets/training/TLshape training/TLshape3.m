    ...
    rectangle('Position',[0 0 12 9],'FaceColor',[0 0 0],'EdgeColor',[0 0 0])
        for k=1:size(Lx,1)
            ...
            if Ly(k,1)==Ly(k,2)&& Ly(k,2)==Ly(k,3)
                rectangle('Position',[min(Lx(k,:)),min(Ly(k,:)),.3,.8],'EdgeColor',rgbval(clrarr(k),:),'LineWidth',6,'Curvature',[1,1]);
            else
                rectangle('Position',[min(Lx(k,:)),min(Ly(k,:))+.3,.8,.3],'EdgeColor',rgbval(clrarr(k),:),'LineWidth',6,'Curvature',[1,1]);
            end
            axis([0 12 0 9]);
        end
        hold on
