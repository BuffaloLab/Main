...
% start posnum for these stimuli 400-499
numlist=(numlist(numlist<600 & numlist>499));

if ~isempty(numlist)
    posnumdum=num2str(max(numlist)+1);
    if (max(numlist)+1)==599
    ...
    rectangle('Position',[0 0 12 9],'FaceColor',[0 0 0],'EdgeColor',[0 0 0])
        for k=1:size(Lx,1)
            ...
            if Ly(k,1)==Ly(k,2)&& Ly(k,2)==Ly(k,3)
                rectangle('Position',[min(Lx(k,:)),min(Ly(k,:)),.57,.63],'EdgeColor',rgbval(clrarr(k),:),'LineWidth',6,'Curvature',[1,1]);
            else
                rectangle('Position',[min(Lx(k,:)),min(Ly(k,:))+.3,.63,.57],'EdgeColor',rgbval(clrarr(k),:),'LineWidth',6,'Curvature',[1,1]);
            end
            axis([0 12 0 9]);
        end
        hold on
        axis off
        Tpos=pos(12,:);
        rectangle('Position',[pos(12,1),pos(12,2)-.6,.60,.60],'EdgeColor',rgbval(Tclr,:),'LineWidth',6,'Curvature',[1,1]);
