%% MAKES SETS # 400-499,  offset .35 and .25,  60 total blocks
clc
clear
pack
...
numlist=(numlist(numlist<500 & numlist>399));

if ~isempty(numlist)
    posnumdum=num2str(max(numlist)+1);
    if (max(numlist)+1)==500
...
for l = 1:732 %change last number to change # of screens produced
    ...
    for poslop=1:size(pos,1)-1
        ...
        Larrx=[r (r+.6) (r+.35) (r+.35) ; (r+.6) (r+.6) r (r+.6); r (r+.6) r r; r (r+.6) r r];
        Larry=[p p p (p-.6); (p-.6) p (p-.35) (p-.35); (p-.35) (p-.35) p (p-.6); (p-.25) (p-.25) p (p-.6)];
...
disp(['Generated item file ' 'TL' posnum '.itm'])
    ...
    disp (['file ' 'TL' posnum '.itm already exists, NO .ITM file created'])
end