...
%   Version 4.2.1.1
%
% multiple errors in openNSx fixed by NKillian on 4/4/2012
%
...
NSx.MetaTags.openNSxver = '4.2.1.1';
...
if strcmp(ReadData, 'read')
    ...
    while ftell(FID) < fData
        ...
        % NKillian 4/4/2012:
        firstchan = find(NSx.MetaTags.ChannelID == min(Chan));
        lastchan = find(NSx.MetaTags.ChannelID == max(Chan));        
        allchans = NSx.MetaTags.ChannelID(firstchan:lastchan);
        [~,keepind] = intersect(allchans,Chan);       
        ReadElec = length(allchans);
        % fread here has to read all of the data in between and then remove unwanted channels, NK 4/4/2012
        % precision is short=>short  by default
        ...
        if pausedFile
            ...
            NSx.Data{segIDX}    = NSx.Data{segIDX}(keepind,:);
        else
